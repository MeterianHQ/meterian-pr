...
class PrChangeTest(unittest.TestCase):
    ...
    def test_should_merge_pr_change(self):
        changeOne = PrChange(
            "uuid",
            [ Dependency("dotnet", "System.Net.Http", "4.3.0") ],
            [ FilesystemChange("src/mylibs/alpha.csproj", b"content") ],
            self.__create_report(Dependency("dotnet", "System.Net.Http", "4.3.0"))
        )
        changeTwo = PrChange(
            "uuid",
            [ Dependency("dotnet", "System.Text.RegularExpressions", "4.3.0") ],
            [ FilesystemChange("src/mylib/beta.csproj", b"content") ],
            self.__create_report(Dependency("dotnet", "System.Text.RegularExpressions", "4.3.0"))
        )

        changeOne.merge(changeTwo)

        self.assertEqual(2, len(changeOne.dependencies))
        self.assertTrue(Dependency("dotnet", "System.Text.RegularExpressions", "4.3.0") in changeOne.dependencies)
        ...
        self.assertEqual(2, len(changeOne.pr_report["autofix"]["changes"]))
